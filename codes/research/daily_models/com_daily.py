...
def get_trend_signal(df_mkt: pd.DataFrame, k1: float = 0.2, k2: float = 0.2, std_weight=0.2, sod: bool = True):
    ...
    for item in df_mkt[[ref_price, 'hh', 'lc', 'hc', 'll', 'accstd', 'openPrice']].values:
        ...
        if _pos:  # has position
            for long_short, price, ts, vol in _pos:
                if item[-1] == long_short or item[
                    -1] == define.NO_SIGNAL or order_vol == 0:  # hv pos and same signal direction or no signal, pos not change,update account fee and mv
...
def backtest(start_date: str = '20210403', end_date: str = '20220419', save_factor: bool = False):
    ...
    if _debug:
        ...
        product_id_lst = ['P', 'rb', 'm', 'eg', 'fu', 'lu', 'i', 'j', 'TA', 'wr', 'cu', 'ag', 'cu', 'WH', 'lh']
        k1_lst = [0.1, 0.2, 0.3, 0.5]
        k2_lst = [0.1, 0.2, 0.3, 0.5]
            ...
            for std_weight in std_weight_lst:
                for product_id in product_id_lst:
                    for lag_window in lag_windows_lst:
                        strategy_name = "{0}_{1}_{2}_{3}_{4}".format(product_id, lag_window, w1, w2, std_weight)
    ...
    bt_df.to_csv('back_test_result_{0}_{1}.csv'.format(start_date, end_date), index=False)


def get_eod_signal(start_date: str = '20200403', end_date: str = '20220420', topN: int = 10) -> pd.DataFrame:
    df_bt = pd.read_csv('back_test_result_{0}_{1}.csv'.format(start_date, end_date))
    df_bt = df_bt[df_bt.acc_return > 0.5]
    df_bt = df_bt[df_bt.sharp_ratio > 3.0]
    df_bt['alpha_sharp_ratio'] = df_bt['sharp_ratio'] - df_bt['bc_sharp_ratio']
    df_bt = df_bt[df_bt.alpha_sharp_ratio > 0]
    df_bt = df_bt[df_bt.max_drawdown < 0.3]
    df_bt = df_bt[df_bt.max_risk_ratio < 0.5]
    df_bt = df_bt.sort_values(by='acc_return', ascending=False)
    df_bt = df_bt.iloc[:topN, :]
    selected_product_id = set(df_bt['product_id'])
    selected_strategy = {}
    for product_id in selected_product_id:
        _df = df_bt[df_bt.product_id == product_id]
        selected_strategy.update({product_id: _df[['acc_return', 'strategy_name']].values})
    pprint.pprint(selected_strategy)
    ret_signal = []
    col_name = []
    for product_id, strategy_info in selected_strategy.items():
        for _val in strategy_info:
            strtegy_name = _val[1]
            product_id, lag_window, w1, w2, std_weight = strtegy_name.split('_')
            signal_option = {'k1': float(w1), 'std_weight': float(std_weight), 'k2': float(w2),
                             'lag_window': int(lag_window)}
            df = get_signal(start_date=start_date, end_date=end_date, product_id=product_id,
                            options=signal_option, sod=False)
            col_name = df.columns
            ret_signal.append(list(df.values[-1]))
    ret_df = pd.DataFrame(ret_signal, columns=col_name)
    ret_df.to_csv('eod_signal.csv', index=False)
    return ret_df
