...
def backtest(start_date: str = '20210707', end_date: str = '20210709', product_id: str = 'rb',
             strategy_name: str = 'T0Signal', param_overwrite: dict = {}):
    """
    :param start_date:
    :param end_date:
    :param product_id:
    ...
    margin_rate = 0.1
    options = {}
    startegy_config = "future_trend_{0}.ini".format(product_id)
    try:
        _strategy_conf = utils.get_path([define.CONF_DIR, startegy_config])
    ...
    options.update(param_overwrite)
    ...
    dates = ['20210708']
    trans_lst = []
    df_daily_cache = utils.get_daily_cache(start_date=start_date, end_date=end_date, product_id=[product_id],
                                           lag_window=5)
    df_daily_cache = df_daily_cache.dropna()
    for idx, trade_date in enumerate(trade_dates):
        _daily_cache = df_daily_cache[df_daily_cache.tradeDate == trade_date].to_dict('record')
        if not _daily_cache:
            continue
        # 盘前数据和因子数据可以用，盘后数据不可用
        options.update({'daily_cache': _daily_cache[0]})
        print('start back test for date:{0}'.format(trade_date))
        start_ts = time.time()
        # _sod_cache = df_daily_cache[df_daily_cache.]
    ...
    f.write(
        "{0}:{1}\ntotal_return:{2}, total_fee:{3},average precision:{4}".format(backtesting_config, result_fname_digest,
                                                                                total_return, total_fee,
