...
class TSModel(object):
    ...
    @timeit
    def train_model(self, product_id='rb'):
        ...
        epoch, rnn, optimizer, train_loss, test_loss = self.load_torch_checkpoint(rnn, optimizer, _rnn_model_path)
        ...
        for i in range(EPOCH):
            ...
            curr_epoch_val_loss = np.mean(step_valid_loss)
            valid_loss.append(curr_epoch_val_loss)
            ...
            if valid_loss and curr_epoch_val_loss < min_valid_loss:
                logger.info("Save model in epoch:{0} with valid_loss:{1}".format(i, valid_loss))
                min_valid_loss = curr_epoch_val_loss
                self.save_torch_checkpoint(epoch, rnn, optimizer, train_loss, min_valid_loss, _rnn_model_path)
...
if __name__ == '__main__':
    ...
    ts_model.train_model(product_id='rb')
