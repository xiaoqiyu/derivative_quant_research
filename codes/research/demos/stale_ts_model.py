#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2023/3/17 17:36
# @Author  : rpyxqi@gmail.com
# @Site    : 
# @File    : stale_ts_model.py

# TODO this module will be deleted when refactor is done, and the new model could be maintain in stable
import torch
...
# LR = 0.001  # 学习率
# SEC_INTERVAL = 60
# TIME_STEP = SEC_INTERVAL * 2  # 步长，一般用不上，写出来就是给自己看的
...
def _get_ts_loader(val=None, targets=None):
    ...
    while idx <= n_features - SEQUENCE:
        try:
            if isinstance(targets, np.ndarray):
                img.append(np.column_stack([val[idx:idx + SEQUENCE], targets[idx:idx + SEQUENCE].reshape(-1, 1)]))
            else:
                img.append(val[idx:idx + SEQUENCE])
...
def get_dataloader(train_ratio=0.7, df=None):
    ...
    # TODO ignore feature processing here
...
def predict_with_lstm(date='2019-12-02', inputs=None, predict_sample={'399005.XSHE': ['002415.XSHE']}):
    ...
    if isinstance(inputs, np.ndarray):
        ...
        rnn_dict = torch.load('LSTM.model')
